...
async function resendSend(apiKey: string, from: string, to: string, subject: string, body: string) {
  try {
    const res = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        from, to, subject: subject || "(sem assunto)",
        text: body,
        html: body.split("\n").join("<br>"),
      }),
    });
...
function pickVariation(vars: any[]): any | null {
  ...
  if (Math.random() < 0.2) return vars[Math.floor(Math.random() * vars.length)]; // exploração 20%
...
async function getChannelState(admin: any, orgId: string, channel: string, now: Date) {
  ...
  let st = data;
  if (!st) {
    const ins = await admin.from("outreach_channel_state").insert({
      organization_id: orgId, channel, day: today(now), week_start: isoWeekStart(now),
    }).select("*").single();
    st = ins.data;
  }
  // rollover
...
async function activeDomain(admin: any, orgId: string) {
  const { data } = await admin.from("outreach_email_domains")
    .select("*").eq("organization_id", orgId).eq("is_active", true)
    .order("health_score", { ascending: false }).limit(1).maybeSingle();
  return data;
}

async function processOrg(admin: any, orgId: string, resendKey: string, fallbackFrom: string, now: Date) {
  const { data: campaigns } = await admin.from("outreach_campaigns")
    .select("*").eq("organization_id", orgId).eq("status", "running");
  if (!campaigns || campaigns.length === 0) return { org: orgId, sent: 0 };

  const state = await getChannelState(admin, orgId, "email", now);
  if (state.status === "circuit_open") return { org: orgId, sent: 0, skipped: "circuit_open" };
  ...
  for (const tgt of targets ?? []) {
    ...
    if (cap != null && state.weekly_sent >= cap) {
      await admin.from("outreach_campaigns").update({ status: "waiting_for_quota" }).eq("id", tgt.campaign_id);
      pausedForQuota.add(tgt.campaign_id);
      continue;
    }
    const camp: any = campMap.get(tgt.campaign_id);
    const steps: any[] = Array.isArray(camp?.steps) ? camp.steps : [];
    if (tgt.current_step >= steps.length) {
      await admin.from("outreach_campaign_targets").update({ status: "completed" }).eq("id", tgt.id);
      continue;
    }
    ...
    if (channel !== "email") { await advance({ last_channel: channel }); continue; } // só email neste marco

    const { data: lead } = await admin.from("outreach_leads").select("*").eq("id", tgt.lead_id).maybeSingle();
    if (!lead || lead.deleted_at || !lead.email) {
      await admin.from("outreach_campaign_targets").update({ status: "stopped" }).eq("id", tgt.id);
      continue;
    }

    const { data: vars } = await admin.from("outreach_template_variations")
      .select("*").eq("template_id", step.template_id).eq("channel", "email");
    const variation = pickVariation(vars ?? []);
    if (!variation) { await advance(); continue; }

    const subject = render(variation.subject || "", lead);
    const body = render(variation.body || "", lead);
    const r = await resendSend(resendKey, from, lead.email, subject, body);

    await admin.from("outreach_messages").insert({
      organization_id: orgId, campaign_id: camp.id, target_id: tgt.id, lead_id: lead.id,
      variation_id: variation.id, channel: "email",
    ...
    if (r.ok) {
      sent++;
      state.daily_sent++; state.weekly_sent++; state.consecutive_failures = 0;
      ...
      if (state.consecutive_failures >= CIRCUIT_THRESHOLD) {
        ...
        await admin.from("outreach_campaigns").update({ status: "paused" }).eq("organization_id", orgId).eq("status", "running");
      }
      // re-tentar daqui a 1h (não avança o passo)
      await admin.from("outreach_campaign_targets").update({ next_action_at: new Date(now.getTime() + 3600_000).toISOString() }).eq("id", tgt.id);
    }

    await admin.from("outreach_channel_state").update({
      daily_sent: state.daily_sent, weekly_sent: state.weekly_sent,
      consecutive_failures: state.consecutive_failures, status: state.status,
    }).eq("id", state.id);

    if (state.status === "circuit_open") break;
  }
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!resendKey) return json({ error: "resend_not_configured" }, 400);
    ...
    // Modo cron (todas as orgs) se o segredo bater certo
    if (cronSecret && provided && provided === cronSecret) {
      const { data: orgs } = await admin.from("outreach_campaigns")
        .select("organization_id").eq("status", "running");
    ...
    // Modo autenticado (org do utilizador)
    ...
    const { data: member } = await admin.from("organization_members")
      .select("id, role").eq("organization_id", orgId).eq("user_id", userId).eq("status", "active").maybeSingle();
