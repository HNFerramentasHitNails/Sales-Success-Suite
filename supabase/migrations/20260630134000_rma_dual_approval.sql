-- Tarefa 8 (brief financeiro) — Segregação de funções e dupla aprovação em devoluções.
--
-- Acima de um limite configurável por organização:
--  - quem CRIOU a devolução não a pode APROVAR (segundo responsável obrigatório);
--  - quem criou a devolução não pode emitir a NOTA DE CRÉDITO (regularizar).
-- Regista-se o aprovador (approved_by/approved_at). Limite NULL = controlo desligado.

ALTER TABLE public.rma
  ADD COLUMN IF NOT EXISTS approved_by uuid,
  ADD COLUMN IF NOT EXISTS approved_at timestamptz;

ALTER TABLE public.organizations
  ADD COLUMN IF NOT EXISTS rma_dual_approval_threshold numeric;

-- Dupla aprovação ao marcar a RMA como aprovada.
CREATE OR REPLACE FUNCTION public.trg_rma_dual_approval()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
AS $function$
DECLARE v_threshold numeric; v_total numeric;
BEGIN
  IF NEW.status = 'approved' AND OLD.status IS DISTINCT FROM 'approved' THEN
    IF auth.uid() IS NOT NULL THEN
      ...
      NEW.approved_at := now();
    END IF;
    SELECT rma_dual_approval_threshold INTO v_threshold FROM public.organizations WHERE id = NEW.organization_id;
    IF v_threshold IS NOT NULL THEN
      SELECT total INTO v_total FROM public.orders WHERE id = NEW.order_id;
      IF COALESCE(v_total, 0) > v_threshold AND auth.uid() IS NOT NULL AND auth.uid() = NEW.created_by THEN
        RAISE EXCEPTION 'dual_approval_required'
          USING HINT = 'Devoluções acima do limite exigem aprovação por um segundo responsável (diferente de quem criou).';
      END IF;
    END IF;
  END IF;
  RETURN NEW;
...
DROP TRIGGER IF EXISTS trg_rma_dual_approval ON public.rma;
CREATE TRIGGER trg_rma_dual_approval
  BEFORE UPDATE ON public.rma
  FOR EACH ROW EXECUTE FUNCTION public.trg_rma_dual_approval();

-- process_rma_resolution: adiciona a verificação de segregação de funções.
...
DECLARE
  ...
  v_threshold numeric;
BEGIN
  SELECT id, organization_id, customer_id, order_id, status, resolution, created_by
  ...
  -- Segregação de funções: acima do limite, quem criou a devolução não a pode regularizar.
  SELECT rma_dual_approval_threshold INTO v_threshold FROM public.organizations WHERE id = v_rma.organization_id;
  IF v_threshold IS NOT NULL AND COALESCE(v_order.total,0) > v_threshold
     AND auth.uid() IS NOT NULL AND auth.uid() = v_rma.created_by THEN
    RAISE EXCEPTION 'segregation_of_duties'
      USING HINT = 'Quem criou a devolução não pode emitir a nota de crédito acima do limite.';
  END IF;
