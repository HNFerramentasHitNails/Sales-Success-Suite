-- Impede regularizar duas vezes a mesma encomenda (ex.: criar uma segunda devolução para
-- a mesma encomenda depois de já ter sido reembolsada/creditada). A checagem por rma_id já
-- existente cobre repetir a MESMA devolução; esta cobre uma devolução DIFERENTE na mesma encomenda.
...
DECLARE
  ...
  v_threshold numeric; v_customer_id uuid; v_order_already_refunded boolean;
BEGIN
  ...
  SELECT id, subtotal, tax_total, total, currency, assigned_member_id, order_number, customer_id
    INTO v_order FROM public.orders WHERE id = v_rma.order_id;
  IF v_order.id IS NULL THEN RAISE EXCEPTION 'order_not_found'; END IF;

  -- A encomenda já tem uma nota de crédito de OUTRA devolução: bloqueia duplicar o reembolso.
  SELECT EXISTS(SELECT 1 FROM public.credit_notes WHERE order_id = v_order.id) INTO v_order_already_refunded;
  IF v_order_already_refunded THEN
    RAISE EXCEPTION 'order_already_refunded'
      USING HINT = 'Esta encomenda já tem uma nota de crédito emitida por outra devolução.';
  END IF;

  -- Cliente: usa o da RMA; se não foi escolhido no formulário, cai no da encomenda.
  v_customer_id := COALESCE(v_rma.customer_id, v_order.customer_id);
  ...
  VALUES (v_rma.organization_id, _rma_id, v_order.id, v_customer_id, v_number,
  ...
  IF v_method = 'wallet' AND v_customer_id IS NOT NULL THEN
    PERFORM public._wallet_credit_system(v_rma.organization_id, v_customer_id, v_amount,
            'refund', _rma_id, 'Crédito de devolução ' || v_number);
