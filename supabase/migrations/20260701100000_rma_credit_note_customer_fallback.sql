-- Corrige a nota de crédito de devoluções (RMA): quando a devolução é criada só com
-- "Encomenda" (sem "Cliente" selecionado no formulário), o customer_id da RMA fica NULL,
-- o que fazia o crédito à carteira ser silenciosamente ignorado (o resto — nota de
-- crédito, reposição de stock, reversão de comissão — corria normalmente, daí parecer
-- que "não fazia nada"). Passa a usar o cliente da encomenda como reserva.
...
DECLARE
  ...
  v_threshold numeric; v_customer_id uuid;
BEGIN
