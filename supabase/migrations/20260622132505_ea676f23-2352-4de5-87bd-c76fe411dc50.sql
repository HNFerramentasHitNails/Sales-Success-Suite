...
BEGIN
  ...
  VALUES (v_ord.organization_id, v_ord.customer_id, v_ord.customer_name_raw, v_number, COALESCE(v_ord.order_date::date, current_date), 'draft', 'hub',
          COALESCE(v_ord.subtotal,0) + COALESCE(v_ord.shipping_cost,0), COALESCE(v_ord.tax_total,0), COALESCE(v_ord.total,0), COALESCE(v_ord.currency,'EUR'), v_ord.category, v_ord.sales_rep_id, v_ord.notes, COALESCE(v_ord.shipping_cost,0))
